...
import { Professional, Equipment } from '../types';
            ...
            <div className="bg-white dark:bg-[#1C1C1E] border border-zinc-200 dark:border-zinc-800 rounded-2xl overflow-hidden shadow-sm">
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-zinc-100 dark:divide-zinc-800">
                  <tr className="bg-orange-50/30 dark:bg-orange-950/5">
                    <td className="px-4 py-4">
                      <input type="text" placeholder="Adicionar novo cliente..." id="new-client-nome" className="w-full bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-lg px-3 py-2 outline-none focus:border-orange-500 text-sm font-bold" />
                    </td>
                    <td className="px-4 py-4">
                      <input type="text" placeholder="CNPJ" id="new-client-cnpj" className="w-full bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-lg px-3 py-2 outline-none focus:border-orange-500 text-sm font-mono" />
                    </td>
                    <td className="px-4 py-4 text-right">
                      <button onClick={async () => {
                        const nome = (document.getElementById('new-client-nome') as HTMLInputElement).value;
                        const cnpj = (document.getElementById('new-client-cnpj') as HTMLInputElement).value;
                        if(nome) { 
                          await addClient({nome, cnpj}); 
                          (document.getElementById('new-client-nome') as HTMLInputElement).value = ''; 
                          (document.getElementById('new-client-cnpj') as HTMLInputElement).value = ''; 
                        }
                      }} className="p-2.5 bg-[#ff6b00] text-white hover:bg-[#ff8c00] transition-all rounded-xl shadow-lg shadow-orange-500/20 active:scale-95"><Plus size={18} strokeWidth={3} /></button>
                    </td>
                  </tr>
