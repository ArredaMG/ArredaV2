...
export const Dashboard: React.FC = () => {
  ...
  const calculateProjectTotal = (p: Project) => {
    const latestVersion = p.versions[p.versions.length - 1];
    if (!latestVersion) return 0;
    
    let totalVenda = 0;
    latestVersion.groups.forEach(g => {
      const groupMargin = g.margin !== undefined ? g.margin : latestVersion.defaultMargin;
      g.items.forEach(i => {
        const baseCost = i.unitCost * i.quantity * (i.days || 1);
        const m = groupMargin / 100;
        // Nova Lógica: Venda = Custo / (1 - Margem)
        const valorVenda = m < 1 ? baseCost / (1 - m) : baseCost;
        totalVenda += valorVenda;
      });
    });
    
    const taxRate = (latestVersion.defaultTax || 0) / 100;
    return taxRate < 1 ? totalVenda / (1 - taxRate) : totalVenda;
  };

  const calculateTotalValue = (list: Project[]) => list.reduce((acc, p) => acc + calculateProjectTotal(p), 0);
  ...
  const renderProjectCard = (p: Project) => {
    ...
    const valorFinal = calculateProjectTotal(p);
    
    return (
      <div 
        key={p.id} 
        className="bg-white dark:bg-[#1C1C1E] border border-gray-100 dark:border-gray-800 rounded-2xl p-5 shadow-sm hover:shadow-xl transition-all duration-300 group cursor-pointer relative overflow-hidden" 
        onClick={() => navigate(`/orcamentos/${p.id}`)}
      >
        <div className="flex justify-between items-start mb-3">
          <div className="flex flex-col">
            <h3 className="font-bold text-gray-900 dark:text-gray-100 group-hover:text-[#ff6b00] transition-colors line-clamp-1">{p.title}</h3>
            <span className="text-[10px] font-black uppercase text-zinc-400 mt-0.5 tracking-tighter">{latestVersion.name}</span>
          </div>
          <span className={cn(
        ...
        <p className="text-zinc-500 dark:text-zinc-400 text-xs font-medium mb-4">{p.client}</p>
        
        <div className="flex items-center justify-between mb-4">
          <p className="text-[#ff6b00] dark:text-[#ff8c00] font-mono font-bold text-lg tabular-nums">{formatCurrency(valorFinal)}</p>
        </div>

        <div className="flex items-center justify-between pt-4 border-t border-zinc-100 dark:border-zinc-800/50">
          <div className="text-[10px] text-zinc-400 font-medium">{new Date(latestVersion.date).toLocaleDateString('pt-BR')}</div>
                  ...
                  {filteredProjects.map((p) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-6 py-5 font-mono font-bold text-gray-900 dark:text-white tabular-nums">{formatCurrency(valorFinal)}</td>
                        <td className="px-6 py-5">
                          <span className={cn(
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-start">
                <div className="space-y-6">
                  <div className="flex items-center justify-between px-2">
                    <h3 className="text-xs font-black uppercase tracking-[0.2em] text-zinc-400">Pendente</h3>
                    <span className="bg-zinc-100 dark:bg-zinc-800 text-[10px] font-bold px-2 py-0.5 rounded-full">{pending.length}</span>
                  </div>
                ...
                <div className="space-y-6">
                  <div className="flex items-center justify-between px-2">
                    <h3 className="text-xs font-black uppercase tracking-[0.2em] text-zinc-400">Aprovados</h3>
                    <span className="bg-zinc-100 dark:bg-zinc-800 text-[10px] font-bold px-2 py-0.5 rounded-full">{inProgress.length}</span>
                  </div>
                  <div className="space-y-4">
                ...
                <div className="space-y-6">
                  <div className="flex items-center justify-between px-2">
                    <h3 className="text-xs font-black uppercase tracking-[0.2em] text-zinc-400">Concluídos</h3>
                    <span className="bg-zinc-100 dark:bg-zinc-800 text-[10px] font-bold px-2 py-0.5 rounded-full">{completed.length}</span>
                  </div>
                  <div className="space-y-4">
